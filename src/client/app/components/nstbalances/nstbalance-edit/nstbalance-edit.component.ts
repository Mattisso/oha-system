...
@Component({
  ...
  styleUrls: ['./nstbalance-edit.component.scss']
})
export class NstbalanceEditComponent implements OnInit, AfterViewInit, OnDestroy  {
  ...
  constructor(private fb: FormBuilder,
    ...
    private nstbalanceService: NstbalanceService,
...
this.getBalance(id);
  ...
  getBalance(id: string) {
    this.nstbalanceService.getBalance(id)
        .subscribe(
  ...
  deleteBalance(): void {
    if (this.balance.id === null) {
        ...
        if (confirm(`Really delete the balance: ${this.balance.IntitulCompte}?`)) {
            this.nstbalanceService.deleteBalance(this.balance.id)
                .subscribe(
