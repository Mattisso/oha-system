...
var Models = require('../omodels');
  ...
  const outdirname = path.join(__dirname, './data/source/nttcomptebalancedata.json');
const outFilename = path.join(__dirname, './data/target/nttcomptebalancedata.csv');


async.series({
  fileInjson: function (callback) {
    Models.nttCompteBalance.find({},{},function (err, docs) {
      if (err) throw err;
  ...
  fileIncsv: function (callback) {
    ...
    const fields = ['_id','_oexerccompta_id', '_otableauposte_id', '_oreference_id', 'totalSoldeDebit', 'totalSoldeCredit'];
